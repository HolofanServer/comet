...
class DetailedMonitoringCog(commands.Cog):
    ...
    async def on_application_command_error(self, interaction, error):
        ...
        #     })
        logger.error(
            f"Error in command: {interaction.command}\n"
    ...
    async def host_status_monitor(self):
        ...
        try:
            ...
            logger.error(f"Error in host status monitor: {e}")
    ...
    async def ping_monitor(self):
        ...
        try:
            ...
            logger.info(f"Bot Ping - Latency: {latency}ms")
        except Exception as e:
            # sentry_sdk.capture_exception(e)
            logger.error(f"Error in ping monitor: {e}")
    ...
    async def uptime(self, ctx):
        ...
        logger.info(f"Uptime checked: {uptime_duration}")
    ...
    async def hostinfo(self, ctx):
        ...
        try:
            ...
            logger.info("Host info command executed")
        except Exception as e:
            await ctx.send("Error retrieving host information.")
            # sentry_sdk.capture_exception(e)
            logger.error(f"Error in host info command: {e}")

    @commands.hybrid_command(name="issues")
    async def issues(self, ctx):
        """Sentryの問題一覧を表示するコマンド"""
        # Sentryの機能は無効化済み - 単純な応答を返すだけ
        embed = discord.Embed(
            title="Sentry Issues",
            description="この機能は現在無効化されています。",
            color=discord.Color.dark_grey()
        )
        await ctx.send(embed=embed)
        logger.info("Issues command called - Sentry is disabled")
